...
- `ModuleSmoke/`：HistoryVulcan UI/Service 双模式装载、命令、三个页面（图谱独立窗口、分支历史内嵌）、卸载与热重载测试。
...
产物只覆盖 `z-Publish/HistoryJanus-vX.Y.Z/`。完整 Debug/Release Smoke、ModuleSmoke
及正式发布校验由宿主 `vulcan.dev.submit` / `finish` 执行；本项目只保留候选构建入口。Diana 不发布。
