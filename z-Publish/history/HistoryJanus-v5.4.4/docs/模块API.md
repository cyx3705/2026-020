...
本文件描述活动源的 `5.4.4` 候选合同；只有用户另行授权正式发布后，同版本 manifest 和二进制才会提升到
...
| `projops` | 项目操作 | 左侧（宽度占比 `0.38`） | 页面描述中的提交、推送与治理操作 |
...
| `janus.ui.data` | 只读 | 按 `view` 返回页面数据（`projects` / `graph` / `history` / `rules` / `github`）；`projects` 返回页面行字段 `name`、`isClean`、`subject` |
...
- 模块卸载时宿主撤销所有来源为 `module:HistoryJanus` 的命令并移除三个描述化页面。
